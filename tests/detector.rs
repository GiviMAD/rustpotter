...
fn it_can_detect_wakewords() {
    ...
    let live_audio_simulation = get_audio_with_two_wakewords(sample_rate, bits_per_sample, sample_1_path, sample_2_path);
    ...
    assert!(detected_wakewords[0].avg_score > 0.2);
    assert_eq!(detected_wakewords[0].score, 0.72954464);
    assert!(detected_wakewords[1].avg_score > 0.2);
...
fn it_can_detect_wakewords_while_applying_gain_normalizer_audio_filter() {
    ...
    let sample_1_path = dir.to_owned() + "/tests/resources/oye_casa_g_1.wav";
    let sample_2_path = dir.to_owned() + "/tests/resources/oye_casa_g_2.wav";
    let live_audio_simulation = get_audio_with_two_wakewords_with_gain(sample_rate, bits_per_sample, sample_1_path, sample_2_path, 3.5, 30.3);
    let detected_wakewords = live_audio_simulation
        .chunks_exact(rustpotter.get_bytes_per_frame())
        .filter_map(|audio_buffer| rustpotter.process_byte_buffer(audio_buffer.to_vec()))
        .collect::<Vec<_>>();
...
fn get_audio_with_two_wakewords(sample_rate: usize, bits_per_sample: u16, sample_1_path: String, sample_2_path: String) -> Vec<u8> {
    get_audio_with_two_wakewords_with_gain(sample_rate, bits_per_sample, sample_1_path, sample_2_path, 1., 1.)
}
fn get_audio_with_two_wakewords_with_gain(sample_rate: usize, bits_per_sample: u16, sample_1_path: String, sample_2_path: String, sample_1_gain: f32, sample_2_gain: f32) -> Vec<u8> {
...
fn read_wav_buffer(path: &str, gain: f32) -> Vec<u8> {
    ...
    // remove wav header
    buffer.drain(0..44);
    let buffer_with_gain = buffer.chunks_exact(2)
    .map(|bytes| i16::to_le_bytes((i16::from_le_bytes([bytes[0], bytes[1]]) as f32 * gain) as i16))
